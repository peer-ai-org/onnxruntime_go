...
def save_model(model, output_filename):
    ...
    torch.onnx.export(model, dummy_input, output_filename,
        input_names=input_names, output_names=output_names, use_external_data_format=True)
    return None
...
def main():
    ...
    model.eval()
    with torch.no_grad():
        save_model(model, "example_network_external.onnx")
        save_sample_json(model, "example_network_results_external.json")
        print_sample(model)
    print("Done!")
